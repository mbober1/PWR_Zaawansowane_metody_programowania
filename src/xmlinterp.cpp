...
void XMLInterp4Config::ProcessCubeAttrs(const xercesc::Attributes  &rAttrs)
{
 ...
 if (IStrmScale.fail() || IStrmShift.fail() || IStrmRot.fail() || IStrmTrans.fail()) 
 {
  cerr << " Blad!!!" << endl;
 }

 config.add_mobile_obj(sValue_Name, Shift, Scale, Rot, Trans, sValue_RGB);
 ...
 */
bool read_file(const char* filename, Configuration &config)
{
  try 
  {
